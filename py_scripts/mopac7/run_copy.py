...
def main():
    ...
    clear_mopac_outputs()
    run_mopac_exe()
    return parse_mopac_output(len(atoms))
...
def parse_mopac_output(n_of_atoms):
    with open(MOPAC7_PATH / "FOR006") as f:
        output = f.read()

    read_from = output.rfind("TOTAL ENERGY")
    read_to = output[read_from:].find("EV")+read_from
    total_energy = float(output[read_from:read_to].split()[-1])

    read_from = output.rfind("CARTESIAN COORDINATE DERIVATIVES")
    gradients = np.array(output[read_from:].split())[8:5*n_of_atoms+8]
    gradients_2dim = gradients.reshape(-1, 5)
